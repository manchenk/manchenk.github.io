...
// Этот файл содержит дампы ПЗУ БИС К145ВВ8П.
...
MNEMONICS_VV8P = [
    ...
    [ "R[-1]=M[+0];M[+0]=S",  "M[+0]=S"            ],
    [ "LS=M[+0]+S+L",         "LS=M[+0]+S+L"       ],
    [ "LS=S-1",               "LS=S-1"             ],
    [ "L=R[+0]!=0",           "L=R[+0]!=0"         ],
    [ "S=6",                  "S=6"                ],
    [ "LS=~R[+0]+S+1",        "LS=~R[+0]+S+1"      ],
    [ "LS=~R[+0]+S+L",        "LS=~R[+0]+S+L"      ],
    [ "L=R[+0]!=0&&L==0",     "L=R[+0]!=0&&L==0"   ],
    [ "R[-1]=S=S+L?0:10",     "S=S+L?0:10"         ],
    [ "S=~S",                 "S=~S"               ],
    [ "S=R[+0]+6",            "S=R[+0]+6"          ],
    [ "LS=R[+0]+S;R[+0]=0",   "LS=R[+0]+S;R[+0]=0" ],
    [ "S=S+L",                "S=S+L"              ],
    [ "LS=~R[+0]+1",          "LS=~R[+0]+1"        ],
    [ "S=1;R[+0]=0",          "S=1"                ],
    [ "R[-1]=S=M[+0]",        "S=M[+0]"            ]
...
COMMENTS_VV8P = [
    "", // 00
    "Waiting (M1.11 == 15) and resetting (M1.11 = 1) sync token", // 01
    "", // 02
    "", // 03
    "", // 04
    "", // 05
    ...
    "", // 0D
    "", // 0E
    "Copying data to display register (R0.0-R0.8 <=> M1.0-M1.8)", // 0F
    "", // 10
    "", // 11
    "", // 12
    "", // 13
    ...
    "", // 17
    "", // 18
    "Инверсия знака старшей части (R2.M = -R2.M)", // 19
    "", // 1A
    "Переход на 0x25 ", // 1B
    "", // 1C
    "", // 1D
    "Waiting (M1.11 == 15) and resetting (M1.11 = 1) sync token", // 1E
    "", // 1F
    "", // 20
    "", // 21
    "", // 22
    ...
    "", // 3E
    "", // 3F
    "If M1.11 < 2 then goto 0x6B else goto 0x60", // 40
    "", // 41
    "", // 42
    "", // 43
    "", // 44
    ...
    "", // 74
    "", // 75
    "Инверсия знака младшей части (R0.M = -R0.M)", // 76
    "", // 77
    "", // 78
    "", // 79
    "", // 7A
